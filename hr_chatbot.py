...
def _force_utf8_stdio() -> None:
  """Avoid UnicodeEncodeError on Windows consoles / redirected output."""
...
# This makes it easier to configure GROQ_API_KEY / HR_AI_BACKEND on Windows.
try:
...
OLLAMA_MODEL = os.getenv("OLLAMA_MODEL",  "llama3.2:latest")   # match locally installed model by default

# Groq settings (cloud free tier — https://console.groq.com → free API key)
# IMPORTANT: never hardcode API keys in source code. Use environment variables or a local .env file.
...
class HRChatbot:
    ...
    def _check_ollama(self):
        try:
            ...
            if any(OLLAMA_MODEL.split(":")[0] in m for m in models):
                print(f"[Ollama] '{OLLAMA_MODEL}' ready [OK]")
            else:
                print(f"[Ollama] [WARN] Model not found — run: ollama pull {OLLAMA_MODEL}")
        except Exception:
              print("[Ollama] [WARN] Not running. Start Ollama first.")

    def _check_groq(self):
        if not GROQ_API_KEY:
            print("[Groq] [WARN] GROQ_API_KEY not set. Get a free key at https://console.groq.com")
        else:
    ...
    def _call_ollama(self, system: str, user_prompt: str) -> str:
        ...
        # Local models (llama3.2 etc.) ignore long instruction lists.
        # Strategy: combine system+user into ONE compact prompt with
        # a pre-filled answer stub that forces the model to continue
        # the answer rather than re-explaining the task.
        combined_prompt = (
            f"{system}\n\n"
            f"{user_prompt}"
        )
        ...
        try:
          ...
          # Fallbacks (some environments expose different endpoints)
          if r.status_code == 404:
            ...
            if r.status_code == 404:
              oai_payload = {
                ...
                "temperature": float(payload.get("options", {}).get("temperature", 0.1)),
                "stream": False,
              }
              r = requests.post(
    ...
    def _clean_answer(text: str) -> str:
        """Strip all prompt artifacts and source references from model output."""
        if not text:
            return text

        # Strip leaked prompt headers (start of text)
        text = re.sub(
            r"(?i)^\s*(COMPANY HR POLICY DOCUMENTS|POLICY DOCUMENTS|SOURCE DOCUMENT"
            r"|Policy Documents|HR Assistant\s*:|Answer\s*:)[:\s]*",
            "", text
        )

        # Strip "Source: ..." or "(Source: ...)" anywhere in the text — ALL variants
        text = re.sub(
            r"(?i)\(?source\s*:\s*[^\n)]+\)?\.?",
            "", text
        )
        # Strip filename references like "(10. Leave Policy.docx)" or "Leave Policy.docx"
        text = re.sub(
            r"\(?\d*\.?\s*[\w\s]+\.(docx?|pdf|xlsx?|txt|md)\)?",
            "", text, flags=re.IGNORECASE
        )
        # Strip "According to [filename]" / "As per [filename]" artifacts
        text = re.sub(
    ...
    def _save_meta(self):
      try:
        ...
        print(f"[Meta] [WARN] Could not save meta.json: {e}")
    ...
    def _retrieve(self, query: str, k: int = TOP_K_CHUNKS, analysis: dict | None = None) -> list[dict]:
    ...
    def _analyse_question(message: str) -> dict:
        ...
        if any(w in low for w in [
          ...
          "ta", "da", "ta/da", "allowance", "per diem", "daily allowance",
          ...
          topic = "travel"
        elif any(w in low for w in ["salary", "tankhwa", "talab", "pay", "compensation",
                                     "increment", "bonus", "deduction", "net pay"]):
            topic = "salary"
        elif any(w in low for w in ["attendance", "haazri", "hazri", "late", "absent",
                                     "timing", "working hours", "shift", "overtime", "ot"]):
            topic = "attendance"
        elif any(w in low for w in ["grievance", "shikayat", "complaint", "report",
                                     "harass", "disciplin", "warning"]):
            topic = "grievance"
        elif any(w in low for w in ["probation", "confirmation", "promotion", "transfer",
                                     "resignation", "termination", "notice period"]):
            topic = "employment"
        elif any(w in low for w in ["training", "development", "course", "workshop"]):
            topic = "training"
        ...
        specific_signals = [
            ...
            "allowance", "ta", "da", "ta/da", "per diem", "daily allowance",
        ...
        is_specific = any(s in low for s in specific_signals)
        ...
        if topic == "travel":
          if any(w in low for w in ["air", "flight", "airfare", "ticket", "economy", "business class"]):
            travel_mode = "air"
          elif any(w in low for w in ["car", "vehicle", "personal car", "own car", "engine", "cc", "mileage", "fuel", "petrol", "diesel"]):
            travel_mode = "car"
          elif any(w in low for w in ["hotel", "accommodation", "lodging", "stay", "overnight"]):
            travel_mode = "lodging"
          elif any(w in low for w in ["da", "per diem", "daily allowance", "meal", "meals"]):
    ...
    def _expand_query(message: str, analysis: dict) -> str:
        ...
        if topic == "travel":
            ...
            if "manager" in low or "mngr" in low:
                extras.extend(["manager", "management", "grade", "band"])
        elif topic == "leave":
    ...
    def _build_system_prompt(analysis: dict) -> str:
        """
        Conversational, few-shot system prompt optimised for local Ollama models.
        Key insight: showing the model EXAMPLES of good answers works far better
        than giving it a list of rules it will ignore.
        """
        intent = analysis["intent"]
        topic  = analysis["topic"]

        # ── Few-shot examples — teach by showing, not telling ─────────────────
        # These are injected directly into the system prompt so the model
        # learns the expected tone and length from concrete demonstrations.
        examples = {
            "leave": {
                "SPECIFIC": (
                    "Q: How many casual leaves do I get?\n"
                    "A: You get 10 casual leaves per fiscal year (July to June). "
                    "You can't take more than 2 in a row, and they expire at year-end — unused ones don't carry over."
                ),
                "PROCESS": (
                    "Q: How do I apply for casual leave?\n"
                    "A: Send a leave request to your supervisor before the leave date. "
                    "They need to approve it — if you don't get approval first, the absence will be marked as unauthorized and deducted from your pay."
                ),
                "OVERVIEW": (
                    "Q: Tell me about casual leave.\n"
                    "A: MG Apparel gives permanent employees 10 casual leaves per year (July–June). "
                    "These are for urgent personal matters. You can take max 2 at a time, "
                    "unused leaves expire at year-end, and prior supervisor approval is always required."
                ),
                "GENERAL": (
                    "Q: Can I accumulate my casual leaves?\n"
                    "A: No — casual leaves can't be accumulated. "
                    "Whatever you don't use by June 30th simply lapses. There's no encashment either."
                ),
            },
            "attendance": {
                "SPECIFIC": (
                    "Q: What are the working hours?\n"
                    "A: Standard working hours are [as per policy]. Check with HR if you need the exact shift timings for your role."
                ),
                "GENERAL": (
                    "Q: What happens if I'm late?\n"
                    "A: As per the attendance policy, repeated late arrivals may result in deductions. "
                    "The exact thresholds are defined in the policy — contact HR for specifics on your role."
                ),
            },
        }

        # Pick the best example for this topic + intent
        topic_examples = examples.get(topic, examples.get("leave", {}))
        example_pair   = topic_examples.get(intent, topic_examples.get("GENERAL", ""))

        example_block = (
            f"\nExample of a perfect answer:\n{example_pair}\n"
            if example_pair else ""
        )

        travel_addendum = ""
        if topic == "travel":
          travel_addendum = (
            "\nTravel/Allowance guidance:\n"
            "- Do NOT estimate city distances or invent trip facts. If a rule depends on distance/duration/overnight stay/engine CC/grade/travel mode, answer conditionally and ask 1–2 clarifying questions.\n"
            "- Do NOT mix unrelated categories (e.g., airfare entitlements vs car mileage/CC tables). State what each amount applies to and include units (PKR per km/day/night).\n"
            "- If the employee says they are using a personal car and provides engine CC (or CC range), give the policy's PKR-per-km rate (and/or the exact formula) for that CC. Do NOT ask for kilometers unless you are calculating a total amount.\n"
            "- Never claim 'no allowance' unless the policy context explicitly states there is no entitlement.\n"
            "- Prefer exact PKR/RS figures, ceilings, and rates from the policy context (do not say 'approximately' unless the policy itself uses approximate language).\n"
          )

        return (
            "You are the HR Assistant for MG Apparel. You talk to employees like a helpful, "
            "knowledgeable colleague — not like a policy document.\n\n"
            "Rules:\n"
          "1. Answer conversationally. Avoid long bullet-point dumps and policy-style headings. "
          "If the question needs multiple allowances/amounts, a short structured list is OK.\n"
            "2. Give the key fact first, then any important condition — like a real conversation.\n"
          "3. Use ONLY the policy facts provided. Never guess or invent numbers. "
          "If exact PKR/RS amounts are present in the policy context, include them exactly.\n"
            "4. Do NOT mention filenames, document names, or index numbers.\n"
            "5. If not in the policy, say: 'This isn't covered in the current policy — please reach out to HR directly.'\n"
          "6. Keep answers under 4 sentences for simple questions. "
          "For allowance/amount questions, you may use 6–10 short lines to list exact entitlements and conditions.\n"
          "7. ONLY reply in professional English. NEVER reply in Roman Urdu, even if asked in Roman Urdu.\n"
          f"{example_block}"
          f"{travel_addendum}"
        )
    ...
    def chat(self, message: str, history: list[dict]) -> str:
        ...
        if analysis.get("topic") == "travel" and analysis.get("is_specific"):
            ...
            asks_allowance = any(t in low for t in [
                "allowance", "ta", "da", "ta/da", "per diem", "daily allowance",
                "reimbursement", "claim", "expense", "amount", "how much", "kitna", "kitni",
            ])
            if asks_allowance and not (analysis.get("travel_mode") or "").strip():
                return (
                    "To determine the exact allowance according to the policy, please confirm 3 things: "
                    "(1) Travel mode: personal car / company car / taxi / bus-rail / flight? "
                    "(2) Is there an overnight stay (hotel) or a same-day return? "
                    "(3) If using a personal car, what is the engine CC (e.g., 1000/1300/1800/2000+)? "
                    "Then I can share the exact PKR rates/ceilings from the Business Travel Policy."
                )
        ...
        if analysis.get("topic") == "travel" and analysis.get("is_specific"):
          mode = analysis.get("travel_mode") or ""
          if mode == "air":
            extra_q = " airfare ticket flight economy business class entitlement"
          elif mode == "car":
            extra_q = " mileage per km cc engine fuel reimbursement"
          elif mode == "lodging":
            extra_q = " hotel accommodation lodging ceiling per night"
          elif mode == "per_diem":
            extra_q = " daily allowance per diem meal allowance per day"
          else:
            extra_q = " daily allowance per diem hotel accommodation meal mileage airfare ticket ceiling limit reimbursement"
        ...
        if context:
            ...
            facts_block = (
                "[Extracted employee/trip details]\n" + "\n".join(facts) + "\n\n"
            ) if facts else ""
  ...
  <div class="panel on" id="panel-chat">
    ...
    <div class="msgs" id="msgs">
      <div class="welcome">
        ...
        <p>I read your company's HR documents and answer policy questions in detail. Ask me about leave, attendance, benefits, grievances, SOPs, or any HR topic.</p>
        <div class="wtags">
    ...
    <div class="ibar">
      <div class="chips">
        ...
        <button class="chip" onclick="ch(this)">How to apply for sick leave?</button>
        <button class="chip" onclick="ch(this)">What are the rules for casual leave?</button>
...
async function checkStatus(){
  try{
    ...
    if(d.warn){
      w.textContent = d.warn_msg || '⚠️ AI backend not reachable. Start Ollama or set your Groq API key.';
...
def api_status():
    ...
    if AI_BACKEND == "groq":
        ...
        try:
            ...
            if not models:
                warn = True
                warn_msg = "⚠️ Ollama is running but no models are installed. Run: ollama pull llama3.2"
        except Exception:
            warn = True
